...
var NDThemes = new function () {
	this.SetCurrentTheme = function (themeID, userSelected) {
		...
		if (newUserSelectedThemeID != undefined && newUserSelectedThemeID.startsWith("Auto:")) {
			...
			if (this.systemThemeChangeEventHandler == undefined) {
				...
				window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", this.systemThemeChangeEventHandler);
			}
		}
		...
		if (newEffectiveThemeID != oldEffectiveThemeID) {
			document.dispatchEvent(new CustomEvent("NDEffectiveThemeChange", {
				detail: {
		...
		if (this.availableThemes == undefined) {
			...
			if (this.userSelectedThemeID != undefined) {
				for (var i = 0; i < this.availableThemes.length; i++) {
					if (this.userSelectedThemeID == this.availableThemes[i][1]) {
			...
			if (!currentThemeIsInAvailableThemes) {
				...
				if (this.userSelectedThemeHistory != undefined) {
					for (var ui = 0; ui < this.userSelectedThemeHistory.length; ui++) {
						for (var ai = 0; ai < this.availableThemes.length; ai++) {
							if (this.availableThemes[ai][1] == this.userSelectedThemeHistory[ui]) {
				...
				if (newThemeID == undefined) {
					newThemeID = this.availableThemes[0][1];
				}
	...
	this.LoadUserSelectedThemeHistory = function () {
		var joinedString = window.localStorage.getItem("NDThemes.UserSelectedThemeHistory");
	...
	this.SaveUserSelectedThemeHistory = function () {
		if (this.userSelectedThemeHistory == undefined) {
			window.localStorage.removeItem("NDThemes.UserSelectedThemeHistory");
		} else {
			var joinedString = this.userSelectedThemeHistory.join(",");
			window.localStorage.setItem("NDThemes.UserSelectedThemeHistory", joinedString);
		}
	};
	this.AddToUserSelectedThemeHistory = function (themeID) {
		if (this.userSelectedThemeHistory == undefined) {
			...
			if (this.userSelectedThemeHistory.length > 10) {
				this.userSelectedThemeHistory.splice(10);
			}
		}
	};
	...
	this.OnSystemThemeChange = function (event) {
		if (this.userSelectedThemeID != undefined && this.userSelectedThemeID.startsWith("Auto:")) {
...
var NDThemeSwitcher = new function () {
	...
	this.IsNeeded = function () {
		return (NDThemes.availableThemes != undefined && NDThemes.availableThemes.length > 1);
	};
	...
	this.BuildMenu = function () {
		...
		if (NDThemes.availableThemes != undefined) {
			for (var i = 0; i < NDThemes.availableThemes.length; i++) {
				...
				var safeThemeID = theme[1].replace(/[:\/]/g, "");
				html += "<a class=\"TSEntry TSEntry_" + safeThemeID + "Theme\"";
				if (theme[1] == NDThemes.userSelectedThemeID) {
					html += " id=\"TSSelectedEntry\"";
				}
				html += " href=\"javascript:NDThemeSwitcher.OnMenuEntryClick('" + theme[1] + "');\">" + "<div class=\"TSEntryIcon\"></div>" + "<div class=\"TSEntryName\">" + theme[0] + "</div>" + "</a>";
			}
		}
...
NDThemes.SetAvailableThemes([
	["Dark", "DarkTheme"],
	["Light", "LightTheme"],
	["Auto", "Auto:LightTheme/DarkTheme"]
]);
