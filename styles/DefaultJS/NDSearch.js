...
var NDSearch = new function () {
    this.Start = function () {
        ...
        this.initialTimeoutStatus = 1;
        this.topLevelEntryCount = 0;
        this.visibleEntryCount = 0;
        this.openParents = [];
        this.keyboardSelectionIndex = -1;
        this.moreResultsThreshold = 25;
        this.allPrefixesStatus = 1;
        this.prefixObjects = {};
    ...
    this.Update = function () {
        ...
        var forceExpansion = (this.TotalMatchesGreaterThan(searchInterpretations, searchInterpretationPrefixes, 10) === false);
    ...
    this.LoadMoreResults = function () {
        this.moreResultsThreshold = this.visibleEntryCount + 25;
        this.domSearchField.focus();
        this.Update();
    };
    ...
    this.OnSearchFieldFocus = function (event) {
        if (!this.SearchFieldIsActive()) {
            ...
            if (this.allPrefixesStatus == 1) {
                this.allPrefixesStatus = 2;
    ...
    this.OnSearchFieldChange = function (event) {
        this.keyboardSelectionIndex = -1;
        if (this.initialTimeoutStatus == 3) {
            if (this.updateTimeout == undefined) {
            ...
            var searchInterpretations = this.GetSearchInterpretations();
            if (searchInterpretations.length != 0 && this.allPrefixesStatus == 3 && this.GetMatchingPrefixes(searchInterpretations).length <= 1) {
                if (this.initialTimeoutStatus == 2) {
                    clearTimeout(this.initialTimeout);
                    this.initialTimeout = undefined;
                }
                this.initialTimeoutStatus = 3;
                this.Update();
            } else if (this.initialTimeoutStatus == 1) {
                this.initialTimeoutStatus = 2;
                this.initialTimeout = setTimeout(function () {
                    if (NDSearch.initialTimeoutStatus == 2) {
                        clearTimeout(NDSearch.initialTimeout);
                        NDSearch.initialTimeout = undefined;
                        NDSearch.initialTimeoutStatus = 3;
                        NDSearch.Update();
                    }
                }, 1250);
            }
    ...
    this.GetSearchInterpretations = function () {
        ...
        normalizedSearchText = normalizedSearchText.replace(/ $/, "");
        normalizedSearchText = normalizedSearchText.replace(/([^a-z0-9_]) /g, "$1");
    ...
    this.GetMatchingPrefixes = function (searchTextArray) {
        ...
        for (var i = 0; i < searchTextArray.length; i++) {
            ...
            if (searchPrefix != undefined && searchPrefix != "") {
                ...
                while (prefixIndex < this.allPrefixes.length) {
                    if (this.allPrefixes[prefixIndex].length >= searchPrefix.length && this.allPrefixes[prefixIndex].substring(0, searchPrefix.length) == searchPrefix) {
        ...
        for (var i = 1; i < matchingPrefixes.length;) {
    ...
    this.GetAllPrefixesIndex = function (prefix) {
        ...
        var firstIndex = 0;
        var lastIndex = this.allPrefixes.length - 1;
        for (; ;) {
    ...
    this.KeywordMatchesInterpretations = function (keywordObject, interpretations) {
        for (var i = 0; i < interpretations.length; i++) {
            ...
            if (interpretation.length <= keywordObject[1].length) {
                if (keywordObject[1].indexOf(interpretation) != -1) {
                    return true;
                }
            } else {
                if (interpretation.indexOf(keywordObject[1]) != -1) {
                    return true;
                }
            }
        }
    ...
    this.MemberMatchesInterpretations = function (memberObject, interpretations) {
        for (var i = 0; i < interpretations.length; i++) {
            var interpretation = interpretations[i];
            if (memberObject[3].indexOf(interpretation) != -1) {
                return true;
            }
        }
        return false;
    ...
    this.TotalMatchesGreaterThan = function (searchInterpretations, searchInterpretationPrefixes, maximum) {
        ...
        for (var p = 0; p < searchInterpretationPrefixes.length; p++) {
            ...
            if (this.prefixObjects[prefix] == undefined || this.prefixObjects[prefix][2] == false) {
                return undefined;
            }
            var keywordObjects = this.prefixObjects[prefix][1];
            for (var k = 0; k < keywordObjects.length; k++) {
                ...
                if (this.KeywordMatchesInterpretations(keywordObject, searchInterpretations)) {
                    var memberObjects = keywordObject[2];
    ...
    this.BuildResults = function (searchInterpretations, searchInterpretationPrefixes, favorClasses, forceExpansion) {
        ...
        for (var p = 0; p < searchInterpretationPrefixes.length; p++) {
            ...
            if (this.prefixObjects[prefix] == undefined) {
                ...
                break;
            } else if (this.prefixObjects[prefix][2] == false) {
                addSearchingStatus = true;
                break;
            }
            var keywordObjects = this.prefixObjects[prefix][1];
    ...
    this.BuildKeyword = function (keywordObject, searchInterpretations, favorClasses, forceExpansion) {
        ...
        for (var i = 0; i < keywordObject[2].length; i++) {
            var memberObject = keywordObject[2][i];
            if (this.MemberMatchesInterpretations(memberObject, searchInterpretations)) {
        ...
        if (memberMatches == 0) {
            return "";
        } else if (memberMatches == 1 && lastMatchingMemberObject[3] == keywordObject[1]) {
            var selected = (this.keyboardSelectionIndex == this.visibleEntryCount);
            var commentType = lastMatchingMemberObject[4];
            var target;
            if (favorClasses && lastMatchingMemberObject[6] != undefined) {
                target = lastMatchingMemberObject[6];
            } else {
                target = lastMatchingMemberObject[5];
            }
            var html = "<a class=\"SeEntry T" + commentType + "\" " + (selected ? "id=\"SeSelectedEntry\" " : "") + "href=\"#" + target + "\">" + "<div class=\"SeEntryIcon\"></div>" + lastMatchingMemberObject[1];
            if (lastMatchingMemberObject[0] != undefined || lastMatchingMemberObject[2] != undefined) {
                html += "<span class=\"SeQualifier\">";
                if (lastMatchingMemberObject[0] != undefined) {
                    html += ", " + lastMatchingMemberObject[0];
                }
                if (lastMatchingMemberObject[2] != undefined) {
                    html += ", " + lastMatchingMemberObject[2];
                }
                html += "</span>";
            }
            html += "</a>";
            ...
            var html = "<a class=\"SeEntry SeParent " + openClosed + "\" " + (selected ? "id=\"SeSelectedEntry\" " : "") + "href=\"javascript:NDSearch.ToggleParent(" + this.topLevelEntryCount + ",false)\">" + "<div class=\"SeEntryIcon\"></div>" + keywordObject[0] + " <span class=\"SeChildCount\">(" + memberMatches + ")</span>" + "</a>";
            this.topLevelEntryCount++;
            this.visibleEntryCount++;
            if (openClosed == "open") {
                ...
                for (var i = 0; i < keywordObject[2].length; i++) {
                    ...
                    if (this.MemberMatchesInterpretations(memberObject, searchInterpretations)) {
                        var selected = (this.keyboardSelectionIndex == this.visibleEntryCount);
                        var commentType = memberObject[4];
                        var target;
                        if (favorClasses && memberObject[6] != undefined) {
                            target = memberObject[6];
                        } else {
                            target = memberObject[5];
                        }
                        html += "<a class=\"SeEntry T" + commentType + "\" " + (selected ? "id=\"SeSelectedEntry\" " : "") + "href=\"#" + target + "\">" + "<div class=\"SeEntryIcon\"></div>" + memberObject[1];
                        if (memberObject[0] != undefined || memberObject[2] != undefined) {
                            html += "<span class=\"SeQualifier\">";
                            if (memberObject[0] != undefined) {
                                html += ", " + memberObject[0];
                            }
                            if (memberObject[2] != undefined) {
                                html += ", " + memberObject[2];
                            }
                            html += "</span>";
                        }
    ...
    this.BuildSearchingStatus = function () {
        return "<div class=\"SeStatus Searching\">" + "Searching..." + "</div>";
    };
    this.BuildNoMatchesStatus = function () {
        return "<div class=\"SeStatus NoResults\">" + "No Matches" + "</div>";
    };
    this.BuildMoreResultsEntry = function () {
        var selected = (this.keyboardSelectionIndex == this.visibleEntryCount);
        var html = "<a class=\"SeEntry MoreResults\" " + (selected ? "id=\"SeSelectedEntry\" " : "") + "href=\"javascript:NDSearch.LoadMoreResults();\">" + "<div class=\"SeEntryIcon\"></div>" + "More Results..." + "</a>";
        this.visibleEntryCount++;
        this.topLevelEntryCount++;
        return html;
    };
    ...
    this.PositionResults = function () {
        ...
        var maxWidth = urX - 25;
        var maxHeight = window.innerHeight - urY - 25;
    ...
    this.LoadPrefixData = function (prefix) {
        if (this.prefixObjects[prefix] == undefined) {
            var prefixObject = [];
            prefixObject[0] = prefix;
            prefixObject[2] = false;
            prefixObject[3] = "NDPrefixLoader_" + this.PrefixToHex(prefix);
            this.prefixObjects[prefix] = prefixObject;
            NDCore.LoadJavaScript(this.PrefixToDataFile(prefix), prefixObject[3]);
        }
    };
    this.OnPrefixDataLoaded = function (prefix, commentTypes, keywordObjects) {
        ...
        for (var k = 0; k < keywordObjects.length; k++) {
            var keywordObject = keywordObjects[k];
            if (keywordObject[1] == undefined) {
                keywordObject[1] = keywordObject[0].toLowerCase();
            }
            for (var m = 0; m < keywordObject[2].length; m++) {
                var memberObject = keywordObject[2][m];
                var commentTypeIndex = memberObject[4];
                memberObject[4] = commentTypes[commentTypeIndex];
                if (memberObject[1] == undefined) {
                    memberObject[1] = keywordObject[0];
                }
                if (memberObject[3] == undefined) {
                    memberObject[3] = memberObject[1].toLowerCase();
                }
            }
        }
        prefixObject[1] = keywordObjects;
        prefixObject[2] = true;
        NDCore.RemoveScriptElement(prefixObject[3]);
        this.Update();
    };
